...
class TTCApi {
    ...
    static readonly QUERY_PREDICTIONS = "?command=predictions&a=<agency>&r=<route>&s=<stop>"
    ...
    async GetRoutes(): Promise<TransitRoute[]> {
        const res = await (await fetch(TTCApi.HOST + TTCApi.QUERY_ROUTELIST.replace("<agency>", this.agency))).text();
    ...
    async GetRoute(tag:string): Promise<TransitRoute>{
        const res = await (await fetch(TTCApi.HOST + TTCApi.QUERY_ROUTE.replace("<agency>", this.agency).replace("<route>", tag))).text();
        const xml = new window.DOMParser().parseFromString(res, "text/xml");
        const json = this.XmlToJson(xml.documentElement);
    ...
    async GetStopPredictions(stopTag:string, routeTag:string): Promise<Prediction[]>{
        const res = await (await fetch(TTCApi.HOST + TTCApi.QUERY_PREDICTIONS.replace("<agency>", this.agency).replace("<route>", routeTag).replace("<stop>",stopTag))).text();
        const xml = new window.DOMParser().parseFromString(res, "text/xml");
        const json = this.XmlToJson(xml.documentElement);
        ...
        if(json.children[0]?.children){
            const predictions: Prediction[] = json.children[0]?.children[0]?.children.map((p: any) => {
