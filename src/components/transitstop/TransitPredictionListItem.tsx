...
const TransitPredictionListItem: React.FC<Props> = ({prediction}) => {
    ...
    const timerText = `${Math.floor(minutes)}:${ ("0" + Math.floor(seconds).toString()).substr(-2)}`

    const arrivalTime = new Date(0);
    arrivalTime.setUTCSeconds(Math.floor(prediction.epochTime/1000))
    
    return <Container><span>{timerText}</span><span>{arrivalTime.toLocaleTimeString()}</span></Container>
}
